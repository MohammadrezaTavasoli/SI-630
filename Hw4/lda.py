...
class Sampler:
    ...
    def get_doc(self, doc_id):
        ...
        """
        return self._doc_tokens[doc_id], self._doc_assign[doc_id],             self._doc_counts[doc_id]
    ...
    def add_doc(self, doc, vocab, doc_id = None, 
        ...
        assert not doc_id in self._doc_tokens, "Doc " + str(doc_id) +             " already added"
        ...
        assert len(self._doc_assign[doc_id]) == len(temp_doc),                "%s != %s" % (str(self._doc_assign[doc_id]), str(temp_doc))
    ...
    def change_topic(self, doc, index, new_topic):
        ...
        assert index < len(self._doc_tokens[doc]),             "Index %i out of range for doc %i (max: %i)" %             (index, doc, len(self._doc_tokens[doc]))

        term = self._doc_tokens[doc][index]
        alpha = self._alpha

        assert index < len(self._doc_assign[doc]),                "Bad index %i for document %i, term %i %s" %                (index, doc, term, str(self._doc_assign[doc]))
        ...
        if old_topic != -1:
            ...
            self._doc_counts[doc][old_topic]=self._doc_counts[doc][old_topic]-1
        ...
        if new_topic != -1:
            ...
            self._doc_counts[doc][new_topic]=self._doc_counts[doc][new_topic]+1
    ...
    def report_topics(self, vocab, outputfilename, limit=50):
        """
    ...
    def sample_probs(self, doc_id, index):
        ...
        alpha[topic] + T[doc_id, topic]   lambda[term] + V[topic, term]
        ------------------------------ * -----------------------------
         sum(alpha) + sum(T[doc_id])          sum(lambda) + sum(V)

        :param doc_id: The document the term appears in
        :param term: The term whose probability you wish to report
        :param topic: The topic probability
        """
        assert self._doc_assign[doc_id][index] == -1,           "Sampling doesn't make sense if this hasn't been unassigned."
        sample_probs = {}
        ...
        for kk in range(self._num_topics):
            ...
            sample_probs[kk]=((self._alpha[kk]+self._doc_counts[doc_id][kk])/(sum(self._alpha)+sum(self._doc_counts[doc_id])))*self._topics.word_in_topic(kk,term)
