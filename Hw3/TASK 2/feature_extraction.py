...
import nltk
import numpy as np
from general_utils import dump_pickle, get_pickle, get_vocab_dict
...
class Sentence(object):
    ...
    def update_child_dependencies(self, curr_transition):
        if curr_transition > 0 and curr_transition <= NUM_DEPS:
            head = self.stack[-1]
            dependent = self.stack[-2]
        elif curr_transition > NUM_DEPS:
            head = self.stack[-2]
    ...
    def get_child_by_index_and_depth(self, token, index, direction,
                                     depth):  # Get child token
        if depth == 0:
            return token
        ...
        if direction == "left":
            if len(token.left_children) > index:
                return self.get_child_by_index_and_depth(
                    self.tokens[token.left_children[index]], index, direction,
                    depth - 1)
            return NULL_TOKEN
        else:
            if len(token.right_children) > index:
                return self.get_child_by_index_and_depth(
                    self.tokens[token.right_children[::-1][index]], index,
                    direction, depth - 1)
            return NULL_TOKEN

    def get_legal_labels(self):
        labels = [1] if len(self.buff) > 0 else [0]
        labels += ([1] * NUM_DEPS) if len(self.stack) > 2 else ([0] * NUM_DEPS)
        labels += ([1] * NUM_DEPS) if len(self.stack) >= 2 else ([0] * NUM_DEPS)
        return labels

    def get_transition_from_current_state(
            self):  # logic to get next transition
        global dep2idx
        if len(self.stack) < 2:
            return SHIFT  # shift
        ...
        if stack_token_1.token_id >= 0 and stack_token_1.head_id == stack_token_0.token_id:  # left arc
            return dep2idx[stack_token_1.dep] + 1
            ...
            return dep2idx[stack_token_0.dep] + 1 + NUM_DEPS  # right arc
        else:
            return SHIFT if len(self.buff) != 0 else None

    def update_state_by_transition(
            self, transition,
            gold=True):  # updates stack, buffer and dependencies
        if transition is not None:
            if transition == SHIFT:  # shift
                self.stack.append(self.buff[0])
                self.buff = self.buff[1:] if len(self.buff) > 1 else []
            elif transition <= NUM_DEPS:  # left arc
                self.dependencies.append(
                    (self.stack[-1], self.stack[-2]
                     )) if gold else self.predicted_dependencies.append(
                         (self.stack[-1], self.stack[-2]))
                self.stack = self.stack[:-2] + self.stack[-1:]
            elif transition > NUM_DEPS:  # right arc
                self.dependencies.append(
...
def convert_string_to_sentence(string):
    ...
    if SettingsConfig.is_lower:
        words = [w.lower() for w in words]
    word_pos_pairs = nltk.pos_tag(words)
    tokens = []
    for token_index, pair in enumerate(word_pos_pairs):
        word, pos = pair
        tokens.append(Token(token_index, word, pos, NULL, -1))
    return Sentence(tokens)


class Dataset(object):
    def __init__(self, model_config, train_data, valid_data, test_data,
                 feature_extractor):
    ...
    def build_embedding_matrix(self):
        ...
        # load word vectors
        word_vectors = {}
        embedding_lines = open(
            os.path.join(DataConfig.data_dir_path, DataConfig.embedding_file),
            "r").readlines()
        ...
        word_embedding_matrix = np.asarray(
            np.random.normal(
                0,
                0.9,
                size=(self.model_config.word_vocab_size,
        ...
        pos_embedding_matrix = np.asarray(
            np.random.normal(
                0,
                0.9,
                size=(self.model_config.pos_vocab_size,
                      self.model_config.embedding_dim)),
            dtype=np.float32)
        ...
        dep_embedding_matrix = np.asarray(
            np.random.normal(
                0,
                0.9,
                size=(self.model_config.dep_vocab_size,
                      self.model_config.embedding_dim)),
            dtype=np.float32)
...
class FeatureExtractor(object):
    ...
    def extract_from_stack_and_buffer(self, sentence, num_words=3):
        ...
        tokens.extend(
            [NULL_TOKEN for _ in range(num_words - len(sentence.stack))])
        tokens.extend(sentence.stack[-num_words:])

        tokens.extend(sentence.buff[:num_words])
        tokens.extend(
            [NULL_TOKEN for _ in range(num_words - len(sentence.buff))])
    ...
    def extract_children_from_stack(self, sentence, num_stack_words=2):
        ...
        for i in range(num_stack_words):
            if len(sentence.stack) > i:
                lc0 = sentence.get_child_by_index_and_depth(
                    sentence.stack[-i - 1], 0, "left", 1)
                rc0 = sentence.get_child_by_index_and_depth(
                    sentence.stack[-i - 1], 0, "right", 1)

                lc1 = sentence.get_child_by_index_and_depth(
                    sentence.stack[-i - 1], 1, "left",
                    1) if lc0 != NULL_TOKEN else NULL_TOKEN
                rc1 = sentence.get_child_by_index_and_depth(
                    sentence.stack[-i - 1], 1, "right",
                    1) if rc0 != NULL_TOKEN else NULL_TOKEN

                llc0 = sentence.get_child_by_index_and_depth(
                    sentence.stack[-i - 1], 0, "left",
                    2) if lc0 != NULL_TOKEN else NULL_TOKEN
                rrc0 = sentence.get_child_by_index_and_depth(
                    sentence.stack[-i - 1], 0, "right",
    ...
    def extract_for_current_state(self, sentence, word2idx, pos2idx, dep2idx):
        direct_tokens = self.extract_from_stack_and_buffer(
            sentence, num_words=3)
        children_tokens = self.extract_children_from_stack(
            sentence, num_stack_words=2)
        ...
        word_input_ids = [
            word2idx[word] if word in word2idx else word2idx[UNK_TOKEN.word]
            for word in word_features
        ]
        pos_input_ids = [
            pos2idx[pos] if pos in pos2idx else pos2idx[UNK_TOKEN.pos]
            for pos in pos_features
        ]
        dep_input_ids = [
            dep2idx[dep] if dep in dep2idx else dep2idx[UNK_TOKEN.dep]
    ...
    def create_instances_for_data(self, data, word2idx, pos2idx, dep2idx):
        ...
        targets = np.zeros(
            (len(lables), self.model_config.num_classes), dtype=np.int32)
...
def load_datasets(load_existing_dump=True):
    ...
    if load_existing_dump:
        ...
        dataset.build_vocab()
        dump_pickle(dataset.word2idx,
                    os.path.join(DataConfig.dump_dir,
                                 DataConfig.word_vocab_file))
        dump_pickle(dataset.pos2idx,
                    os.path.join(DataConfig.dump_dir,
                                 DataConfig.pos_vocab_file))
        dump_pickle(dataset.dep2idx,
                    os.path.join(DataConfig.dump_dir,
        ...
        dump_pickle(dataset.word_embedding_matrix,
                    os.path.join(DataConfig.dump_dir,
