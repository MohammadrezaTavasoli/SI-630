...
def train(save_dir='saved_weights',
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i, (train_x, train_y) in enumerate(get_minibatches([dataset.train_inputs,
            ...
            word_inputs_batch =torch.FloatTensor(word_inputs_batch).long()
            pos_inputs_batch =torch.FloatTensor(pos_inputs_batch).long()  # TODO
            dep_inputs_batch =torch.FloatTensor(dep_inputs_batch).long()
            ...
            loss =loss_func(outputs, labels)  # TODO
            ...
            optimizer.step()
            resultloss.append(loss.detach().cpu().numpy())
            resultacc.append( int((outputs.argmax(1)==labels).sum())/len(labels))
        ...
        parser.train()
    plt.figure()
    plt.plot(range(len(resultloss)),resultloss)
    plt.savefig('resultloss.png')
    plt.figure()
    plt.plot(range(len(resultacc)),resultacc)
    plt.savefig('resultacc.png')
    plt.figure()
    plt.plot(range(len(resultUAS)),resultUAS)
    plt.savefig('resultUAS.png')  
